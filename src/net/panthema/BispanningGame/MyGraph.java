...
class MyGraph extends SparseGraph<Number,MyEdge> {
	...
	/**
	 * Test if e0 closes a cycle of the same color in the graph. If a cycle
	 * is found, then mark edges of cycle with inCircle.
	 */
	...
	/**
	 * Test if e0 closes a cycle of the same color, if it does, mark the
	 * cycle and calculate which of the cycle edges break the cycle
	 * correctly.
	 */	
	boolean markCycleFixes(MyEdge e0)
	{
		...
		if (!markCycle(e0)) return false;
		
		for (MyEdge ei : getEdges())
		{
			if (!ei.inCircle) continue;
	...
	{
		if (markCycle(e0)) throw new RuntimeException("Bad!");
	...
	void calcUniqueExchanges()
	{
		for (MyEdge ei : getEdges())
		{
			...
			if (false)
			{
				...
				if ( isTreeLeaf(x, 1) || isTreeLeaf(y, 1) ||
					 isTreeLeaf(x, 2) || isTreeLeaf(y, 2) )
					ei.isUE = false;
			}
		}
